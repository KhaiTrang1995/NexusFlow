...
public sealed class FlowEngine
{
    ...
    /// <remarks>
    /// <para>
    /// <strong>The two new seams are optional and are not equally forgiving, deliberately.</strong>
    /// A plan declaring a <c>Cache</c> with no <see cref="IResultCache"/> configured dispatches
    /// every time, which is the behaviour it had before stage 5 existed — an unconsulted cache
    /// is slower and never wrong (ADR-0025 §2.3). A plan declaring an <c>Audit</c> with no
    /// <see cref="IAuditSink"/> configured is refused at the audited step, because an unwritten
    /// audit record is a real loss rather than a conservative default (ADR-0025 §2.4) and a
    /// regulated write whose record is the reason it is allowed to happen must not quietly
    /// happen without one.
    /// </para>
    /// </remarks>
    public FlowEngine(
...
#pragma warning restore CA1031
            ...
            // the money moved and nothing can say who moved it, which is the one case
            // FLOWX1032's third remedy says not to ship. A failure here therefore joins the
    ...
    /// <c>docs/diagnostics/FLOWX1032.md</c>'s third remedy is explicit that such a flow should
...
#pragma warning restore CA1031
    ...
    /// stage 5 and stage 7 now reach the same counter, which is the whole point of §9's
