...
FLOWX1033 | FlowX | Error | CompensationRetry is declared on a step with no compensation, so the emitter drops the one policy this runtime executes and the manifest publishes it anyway. Not deleted when P4 lands: no release gives a non-compensable step an undo. [Documentation](https://github.com/votrongdao/FlowX/blob/master/docs/diagnostics/FLOWX1033.md)
FLOWX1032 | FlowX | Warning | Declared policy is not executed by the runtime; every kind a .WithPolicy(...) set declares except CompensationRetry, which is the only policy any code path reads. Deleted when P4 lands the policy engine. [Documentation](https://github.com/votrongdao/FlowX/blob/master/docs/diagnostics/FLOWX1032.md)
FLOWX1031 | FlowX | Warning | Suspension construct is declared but not honoured by the compiler; raised as an error for AwaitSignal, which reaches the plan carrying a timeout the author did not write, and as a warning for Delay and OnTimeout, which reach it not at all. Deleted when WP-63 lands durable suspension and timers. [Documentation](https://github.com/votrongdao/FlowX/blob/master/docs/diagnostics/FLOWX1031.md)
