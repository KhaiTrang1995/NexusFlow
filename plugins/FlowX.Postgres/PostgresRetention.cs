...
public sealed class PostgresRetention
{
    ...
    /// and they are or-ed because either consumer alone is reason enough to keep the row. A
    /// deployment that declares no publisher passes <c>false</c> and the first disjunct
    /// disappears; one that declares no subscriptions passes empty arrays and the second is a
    /// join over nothing. Both empty means nothing consumes the outbox and every row is
    /// discardable, which is the honest answer to a deployment that stages events and reads
    /// them nowhere.
    /// </para>
    /// <para>
    /// <strong>A subscription with no cursor row has read nothing</strong>, which is the
    /// <c>LEFT JOIN</c> and the null test: <see cref="PostgresChangeFeed"/> starts such a
    /// subscription at the beginning of what the outbox still holds, so every row of its type is
    /// owed to it. The comparison is the feed's own, inverted — it offers rows <em>after</em> the
    /// cursor, so a row at or before it has been read.
    /// </para>
    /// <para>
    /// Answered per candidate instance against <c>outbox_event_instance_idx</c> from
    /// <c>0001</c>, and per subscription against <c>change_cursor</c>'s primary key. The
    /// arrays are a parameter rather than a generated <c>IN</c> list, so the statement is a
    /// constant whatever a deployment declares.
    /// </para>
    /// </remarks>
    private const string Owed =
        """
    ...
    /// </para>
    /// </remarks>
    private static Guid CursorIdFor(ChangeSubscription subscription)
    {
