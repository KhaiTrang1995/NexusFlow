 ...
 *
 * AND THEN THE TABLE SPOKE FOR TENANTS IT HAD NOT ASKED ABOUT. The team panel sat outside every
 * boundary over `quota.data ?? []` and `org.data ?? []`, so an organisation with no periods — one
 * where neither read was made at all — was told "nobody carries a revenue number this period",
 * and a refused org chart put every seller under one invented team with the right money in it.
 * Both reads are guarded together now, because both are needed before any of this is a fact.
 */
export function OperationsScreen() {
  ...
  // The quotas and the reporting line, guarded as one: a team is a manager, so neither number on
  // this screen is a fact until both have answered.
  const staffing = together(quota, org)
  ...
  return (
    <Page>
      ...
      <PeriodGate choice={choice} what="capacity" />

      <AsyncBoundary query={staffing} skeletonRows={4} hidden={hasNoPeriod(choice)}>
        {([quotas, chart]) => {
          const capacity = capacityOf(quotas.rows, chart.members)
