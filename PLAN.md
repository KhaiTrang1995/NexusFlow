...
| 19 | **Two broker plugins ship and CI has executed almost none of them.** `FlowX.AzureServiceBus.Tests` runs **7 and skips 22**; `FlowX.Kafka.Tests` runs **1 and skips 21**. Both suites are written against the conformance contract and both skip for want of a live broker, which no workflow stands up — the same shape as the PostgreSQL suite before WP-53, and that one turned out to be hiding real defects. [28 §4](docs/28-Azure-Hosting.md#4-managed-services-every-topology-shares) states the gate: a broker in a test subscription and a conformance run with **zero skips** before either plugin goes on a production path. Nothing enforces that today | Any deployment trusting a bus trigger | repository owner |
