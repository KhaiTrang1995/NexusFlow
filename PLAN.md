...
**Item 7 blocks the next work package**, and is the only one with a deadline set by the
queue rather than by preference. Item 10 blocks no build; it is a decision that will
otherwise be taken by accident, which is the failure mode this file exists to prevent.

Items 2, 3, 5, 8, 9 and 11 are struck through above rather than deleted, because a plan that
silently repairs its own premises teaches nobody what it got wrong.
