...
| 21 | **Transaction pooling may quietly weaken row-level security, and nothing checks.** [28 §4.1](docs/28-Azure-Hosting.md#41-the-connection-ceiling--the-one-that-bites) makes PgBouncer mandatory at scale — thirty replicas at Npgsql's default pool ask for 3 000 connections against a server offering a few hundred, and schema-per-tenant multiplies it. But transaction pooling does not preserve session state, and the tenant context RLS reads is set per connection. The suite that proves isolation has only ever run against a direct connection. The test is cheap and specific: point the tenant isolation tests at the pooled port and require the same result | Any pooled deployment, which is every deployment above a few replicas | repository owner |
