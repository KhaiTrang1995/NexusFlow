...
| 17 | **`flowx_trigger_admitted_total` is declared and nothing produces it.** *The documentation half is closed:* [18 §3](docs/18-Cloud-Native.md#3-autoscaling) printed an HPA rule reading `flowx_trigger_admitted_rate`, which could never have fired, and it now scales on ingress request rate with a caution naming the seven instruments that **do** have producers. [12 §3](docs/12-Observability.md#3-metrics) was already honest and needed nothing. **What is still owed is the decision**: emit the counter at the admission seam, or strike it from `TelemetryNames` along with the `flowx.trigger.kind` and `flowx.trigger.source` attributes. Leaving a named instrument nobody writes is what produced the unfireable rule in the first place | Nothing now. It is a declared surface with no producer | repository owner |
...
**Nine items in this table are live: 12, 15, 16, 17, 19, 20, 21, 22 and 23, plus what is
