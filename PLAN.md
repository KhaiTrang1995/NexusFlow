...
is the only one with a deadline set by the queue rather than by preference. Items 6 and 11
...
silently repairs its own premises teaches nobody what it got wrong. Items 2, 3, 5, 8 and 10
are struck through above rather than deleted, for the same reason.

*Closing item 10 moved one thing about item 6 and did not touch the decision: ADR-0014's
fourth revisit trigger — "P8 approaches manifest v1.0 freeze" — was recorded there as "not
fired, and **not datable**". It is datable now. The trigger still has not fired, and the
choice the row above describes is still the repository owner's and still unmade.*
