...
> [CHECKLIST §5d](CHECKLIST.md#5d-p2--durable-execution--nearly-complete-qr2-and-b8-measured-on-demand-b7-measurable-but-not-judged)
...
| **V5** | cold start ≤ 200 ms, NativeAOT | **met on advisory hardware, and measured at last** — WP-130's harness AOT-publishes the reference sample and times 30 starts: **p50 63.0 ms, p99 102.2 ms** to first flow response, binary 18.0 MiB, RSS 35.4 MiB ([V5-cold-start.md](docs/benchmarks/V5-cold-start.md), shared-container caveats argued there). **Gated the same day**: the `aot` job re-runs the harness on the binary its smoke test accepted and fails over 200 ms at p50 — the tail stays reported-not-gated for the 26 % p99 spread. *This cell said "unreported; nothing times it", then "not gated"* | a cloud-container measurement |
| **V6** | generator allocation ≤ 800,000 B/flow and ≤ 160,000 B/capability | **met, and gated — and improved 5.66 % on 2026-08-15**: 729,311 / 138,657 at 25 flows, 726,564 at 50, after the reader stopped creating 524 SemanticModels for 51 trees (one shared error-factory file was re-bound once per capability); output verified byte-identical over 179 generated files. `check-generator-cost.py` fails the run on a breached ceiling. *ADR-0014's snapshot figures (772,522 / 769,272) are dated 2026-08-10 and stand as history; the ceilings it set are unchanged and are what the gate reads.* [ADR-0014](docs/adr/ADR-0014-derived-error-catalogue-vs-build-budget.md) replaced the ≤ 8 % ratio on 2026-08-10 — a ratio whose denominator is the user's code cannot be passed or failed, only re-argued, which is why the same generator measured +0.4 % on one flow and +67.1 % on two hundred. The `scale-overhead` job still measures the superseded ratio and is still advisory | — |
...
| **Q2** | durable correctness | conformance suite against real Postgres; lease + recovery scan. **The measure — p99 ≤ 15 ms — is still unmeasured**, because WP-50 shipped its chaos rig and not `JournalBenchmarks`. *This cell also said the scenario — a node killed mid-flow — "has never been executed". That expired on 2026-08-01:* `tests/FlowX.Chaos` `SIGKILL`s worker processes at a step boundary and recorded **0 duplicate effects against the guarantee and 0 lost instances over 10 000 flows per arm** ([benchmarks/QR2-chaos.md](docs/benchmarks/QR2-chaos.md)). **Gated**: `chaos.yml` runs it nightly and fails on all four correctness counters — *this cell said "no CI job runs it … WP-62 … not started", stale on the schedule; the true gap, closed 2026-08-14, was the checker never reading the double-takeover counter* |
...
> enforced" to "enforced at build time", leaving one. *WP-59 has since taken that last row
> and the last rule: `06 §5`'s table has no **no — P2** rows at all, and what `ICapability`
> still lists as unenforced is **half** of rule 8 — contract immutability — because
> `FLOWX1006` raised its serialisable half and nothing refuses a mutable contract.*
...
### WP-59 — `FLOWX1006` and the journal payload contract — **shipped**

> **Shipped 2026-08-01, and it was the last package in P2's Must with nothing written for
> it.** `FlowEmitter` emits a payload writer onto a `Durable` flow's dispatcher:
> `DescribeStep` describes every step boundary — what the step produced and the state bag as
> it then stands — `DescribeInput` describes the trigger input, and `RestoreState` reads the
> bag back, because only generated code can name the contract types either way. **None of it
> is emitted for an `Ephemeral` flow** — the contract list is empty before a line is written
> when the profile is not `Durable`, so `RestoreState`, `DescribeInput` and the state-bag
> helper are absent from the type an ephemeral flow loads and budget B2's hard zero is
> untouched by the writer existing. (An ephemeral flow that stages an `.Emit` still gets
> WP-56's outbox `DescribeStep`; that member predates this package.)
>
> **Both halves of the exit criterion are met.** A `Durable` flow whose state bag holds a
> contract no single generated context declares fails the build naming the member
> (`AStateBagMemberOutsideTheGeneratedContextFailsTheBuildNamingIt`,
> `tests/FlowX.Compiler.Tests/PayloadWriterTests.cs`); and every payload the writer hands
> over carries `SensitiveMembers` — ADR-0015's commitment 5, which the note below named as
> the obligation this package was most likely to forget.
>
> **The second exit the note below warned about was not opened, and that is the deliverable's
> real content.** The writer composes no document: `StateBag` hands named values to
> `JournalPayload.OfState` and composition happens *inside* `JournalPayload`, ahead of the
> one `ToJson()` that redacts and stamps. Pinned two ways — the generated source must contain
> neither `Utf8JsonWriter` nor `JsonSerializer.Serialize`
> (`TheStateBagIsHandedToJournalPayloadRatherThanComposedByTheWriter`), and a hostile
> `IPayloadSerializer` receives only the redacted, stamped document
> (`tests/FlowX.Abstractions.Tests/PayloadContractTests.cs`). `JournalMember` has no accessor
> for its value, so composition adds no route to one either.
>
> **`FLOWX1006` is an `error` uniformly**, rather than the determinism set's Warning-then-
> escalate, because it reports only on `Durable` flows — so its trigger *is* the escalation
> condition the rest of the set has to prove, which is `FLOWX1012`'s mutual exclusivity read
> from the other end. **It does not block emission:** the emitter leaves the undeclarable
> contract out and the plan still compiles, so one accurate error does not become a page of
> "does not contain a definition for `Plan`"
> ([docs/diagnostics/FLOWX1006.md](docs/diagnostics/FLOWX1006.md)).
>
> **`schemaVersion` and `IPayloadSerializer` shipped with it**, both from ADR-0008's
> Decision. The stamp rides the redaction pass rather than adding a second walk of the
> document, and it versions the **envelope** — the placeholder, the case-insensitive name
> matching that produces it, the composition of a state bag, and the presence of the stamp
> itself — deliberately not the contract, whose version is already recorded beside every
> payload. `JsonPayloadSerializer.Default` is the JSON case of the seam.
>
> **`flow_instance.input` stops being NULL on every row**, which was an outstanding defect
> recorded before this package. `FlowHost` passed the literal `input: null` and now passes
> `dispatcher.DescribeInput(input)` (`src/FlowX.Hosting/FlowHost.cs:495`). It was not fixable
> in the host — journaling an input needs a `JsonTypeInfo<TIn>` and only generated code can
> name one, which is the same reason `DescribeStep` is on the dispatcher; the reasoning is
> written at the declaration in `src/FlowX.Runtime/IStepDispatcher.cs`. Until this landed a
> replay could not reconstruct what was requested, the audit trail had no record of it, and
> `[Sensitive]` on an input contract protected nothing, because nothing was stored.
>
> **One deviation from the Deliverable row, and it is the first item in it.** *"The generated
> STJ context ADR-0008 chose"* was **not** built and is not owed: FlowX emits no
> `JsonSerializerContext` of its own. The writer reads the contexts the compilation already
> declares and makes membership of one of them the build-time requirement, which is what
> `FLOWX1006`'s fix is — one `[JsonSerializable]` attribute on the author's own context. A
> context FlowX generated would have to be a *second* context declaring the same contracts,
> which is the "two contexts declaring one contract" case the rule itself refuses.
>
...
> member, and during this package both hand-written decorating dispatchers silently inherited
> the default and put `input` back to NULL —
> [open item 12](#9-open-items-blocking-the-plan).
...
>
> *The last clause was a prediction and it did not come true: the writer composes no
> document, so `ToJson()` is still the only exit and there is still one redaction pass. It is
> kept rather than deleted because it is the risk the shipped design was built against, and
> the two assertions that pin the outcome were written from this sentence.*
...
### WP-62 — QR2: the chaos test — **shipped; closed 2026-08-14 by reading the fourth counter**
...
3. **The trigger model is documented past what exists, in two places, and P3 is where both
   are paid for.** Recorded here on 2026-08-01, from the status box
   [09-Trigger-Model](docs/09-Trigger-Model.md) gained on the same day; neither appeared in
   this file or the checklist before.

   **`TriggerKind.Grpc` does not exist at any level.** [09 §4](docs/09-Trigger-Model.md#4-trigger-kinds-and-their-semantics)
   gives `Grpc` its own row with its own delivery, reply and ordering semantics, and there
   is no `Grpc` member of `TriggerKind` (`src/FlowX.Abstractions/Triggers/TriggerEnvelope.cs`),
   no such value in `schemas/flowx.manifest.schema.json`'s **closed** `kind` enum, and no
   attribute. `TriggerKind.Http`'s own summary folds *"REST, gRPC, GraphQL, webhook"* into
   one kind. **This is a ninth kind that was never declared, not a declared one awaiting
   implementation**, and the distinction decides WP-76: shipping gRPC either accepts that it
   reports as `Http` — which is what the abstraction currently says it is — or adds an enum
   member and a schema `enum` value, and the schema is the one ADR-0017 is about to freeze.

   **`FlowExecutionContext.Trigger` is a get-only auto-property that is never assigned**, so
   every running flow reads `default` — kind `Manual`, a null `Source`, no body and a null
   `CorrelationId`. `FlowContext.Trigger` and `IterationScope.Trigger` both forward
   to it faithfully. `TriggerEnvelope` is a type rather than a value in the running system:
   the only places that construct one are `FlowX.Testing`'s `TestFlowContext` and two test
   projects, so **the one thing a test can set is the one thing production never sets**.
   Three transports each producing an envelope with nowhere to put it is the shape of the
   cost, and it lands on WP-70 with `[TriggerKind]` rather than on each plugin.
...
|---|---|---|---|---|
| **P4** Policy and security | **WP-77 … WP-89** · *WP-77 claimed 2026-08-01: stage 4 executes · WP-78 and WP-79 **shipped 2026-08-14**: `Fallback` and `Hedge` declare and execute; [ADR-0078](docs/adr/ADR-0078-stage-four-nests-six-kinds.md) places them — fallback outside retry (its unit is the step), hedge inside retry and outside the other three (its unit is one attempt of several calls) — amending 0024 without touching it. Capability-valued fallback refused-and-recorded with four named blockers; FLOWX1051–1053 claimed · WP-83 **shipped 2026-08-15** — `Consent(purpose)` opens stage 2 (deny-by-default, ordinal, fail-closed on the blank-purpose mistake via FLOWX1057); `Authorize` deliberately **not built as a policy** — the stance machinery is the one reading and a declarable twin would be the two-copies defect; the row's stale clauses corrected and its one live gap (a stage-2 refusal reaches no audit sink) recorded at ADR-0029's fired revisit rather than patched · WP-84 **refused and recorded** — [ADR-0081](docs/adr/ADR-0081-a-batch-has-no-unit-the-engine-can-name.md) names five missing pieces, the decisive one being that a policy cannot suspend an instance: every wait in this platform is a plan node with a journal identity, and a stage-5 policy has none · WP-81 (Validate) and WP-82 (Quota) **shipped 2026-08-15** — DataAnnotations read at build time and emitted into the dispatcher (no runtime reflection, `[Sensitive]` values structurally unable to reach a message), fixed-window quota on an absolute grid with `IQuotaStore` + migration `0015`; refusal categories placed deliberately (`quota_exhausted` is `Forbidden`, not `Unavailable` — waiting inside the period changes nothing). **Thirteen of seventeen catalogue rows now execute.** One number collision resolved at merge: both parallel packages claimed FLOWX1055, the event-schema diagnostic keeps it, Validate's moved to FLOWX1056 — the third time an id was spent twice and the first time the allocator's own warning was proven right in one day · WP-80 **shipped 2026-08-15**: capability-valued fallback — blockers 1 and 4 built ([ADR-0079](docs/adr/ADR-0079-a-fallback-capability-is-a-dispatch-of-its-own.md): a fallback is the step's own dispatch, keyed by the step's index; manifest publishes `fallback:` beside `compensation`), blocker 2 dissolved (no fifth key component — `capability_id` already answers *what ran*, so no migration), blocker 3 **refuted on the merits** (FLOWX1053 asks its question of the fallback too, so ADR-0078 §2.7 survives word for word). Found and fixed in passing: the resume path put a degraded step back on the unwind stack that the live path excluded — one instance, two behaviours, decided by whether a node died* | [ADR-0011](docs/adr/ADR-0011-fixed-policy-stage-order.md), [ADR-0007](docs/adr/ADR-0007-result-over-exceptions.md) | [10-Policy-Framework](docs/10-Policy-Framework.md) — 7 stages, a 16-row policy catalogue, 5-level resolution order, retry-safety flowchart, breaker keys; [15-Security](docs/15-Security.md) — STRIDE, all five authorisation stances | **Mostly recorded, and one third delivered at WP-77 (2026-08-01).** Of the three decisions named here, **one is made**: [ADR-0023](docs/adr/ADR-0023-policy-stages-hook-through-the-plan.md) fixes where a stage hooks in — `ExecutionPlan.HasStepPolicies` plus a resolved `StepNode.StepPolicy`, never a per-step walk of the chain — which is the one that had to be settled to keep B2's hard zero. [ADR-0024](docs/adr/ADR-0024-stage-four-is-a-fixed-nesting.md) fixes the nesting `docs/10` could not, because §2's *"within a stage, an `order` value breaks ties"* names a field `PolicyDescriptor` has never carried and all four stage-4 kinds share one stage. [ADR-0025](docs/adr/ADR-0025-a-partial-policy-engine-executes-stage-four-alone.md) argues each skip separately. **The other two were deliberately not written**, and the reason is the one this repository acts on everywhere else: `IIdempotencyStore` and the audit-record schema are contracts for policies nothing implements — `IStepDispatcher` is type-erased, so an idempotency replay cannot reproduce the value a step put in the state bag, and an audit record the engine can write carries no payload, which makes `redact` vacuous. **Also corrected here: the catalogue has 17 rows and only 9 were declarable** — *11 since WP-78/79 shipped Fallback and Hedge on 2026-08-14* — eight have no builder method, no attribute and no descriptor kind, so they are not a scope question; and the 5-level resolution order has exactly one real level, because no policy attribute exists |
...
| **P9** Hardening and 1.0 | **WP-130 … WP-139** · *WP-130 shipped 2026-08-14: the V5 cold-start harness — p99 102.2 ms against 200, measured, ungated* | all of them | The roadmap table only | **Invented.** The *targets* are unambiguous (V1–V8, Q1–Q8, B1–B13); there is no design. Note eight of the nine samples are a `README.md` and nothing else |
| **Cross-phase** Serverless and latency | **WP-140 … WP-149** · *WP-140 … WP-144 claimed 2026-08-14 — [§6c](#6c-cross-phase--serverless-and-latency-wp-140--wp-142)* | [ADR-0076](docs/adr/ADR-0076-a-host-is-chosen-against-a-capability-contract.md), [ADR-0077](docs/adr/ADR-0077-a-flow-is-dispatched-in-one-of-two-modes.md) | [28-Azure-Hosting](docs/28-Azure-Hosting.md), [18-Cloud-Native](docs/18-Cloud-Native.md) | **Recorded, not invented** — every seam the three packages touch is named from the working tree in §6c |
...
## 6c. Cross-phase — serverless and latency (WP-140 … WP-142)

Opened 2026-08-14 at the repository owner's direction: *the framework must run everywhere
from Docker to serverless, on Azure, with every trigger kind reachable*. The review that
produced these three packages is in the session record; what it found is one structural gap
and one latency ceiling, and neither needs a new execution mode. **`Dispatched` is
deliberately absent** — see the decision recorded at [item 20](#9-open-items-blocking-the-plan).

| Package | The defect | The constraint | Done when |
|---|---|---|---|
| **WP-140** — push admission — **shipped 2026-08-14** | Every sweep's only public entry is `RunOnceAsync()` — pull. A serverless platform *pushes* one item at a time, and there is no door to hand it through: the per-item body is private (`FlowBusScan.DeliverAsync`, `FlowStreamScan`'s equivalent). A Functions host today cannot deliver a Service Bus message to a flow at all | The public per-item entry must **decide and not settle**: `DeliverAsync` both runs the flow and acknowledges through `IBusConsumer`, but a push host's platform settles the message itself — so the seam returns the disposition (started / requeue / dead-letter with reason / deduplicated) and each caller settles in its own way. The pull path keeps `IBusConsumer` and must not change behaviour: same dedup, same poison rules, same journal rows. **This seam is also where `flowx_trigger_admitted_total` finally gets its producer** — [item 17](#9-open-items-blocking-the-plan)'s "one admission point serving every transport" exists once this lands | Calling the push entry with one message produces journal rows identical to what the scan produces for the same message; the disposition for a poison message is dead-letter with ADR-0038's reason and **no** journal row; `TransportEquivalenceTests` extended with the push arm; the admitted counter emits with `kind` and the [12 §3](docs/12-Observability.md#3-metrics) note about "nowhere to emit from" is corrected |
| **WP-141** — `FlowX.Functions` — **shipped 2026-08-14, two divergences at the row's end** | The trigger attributes are read once by `TriggerReader` and turned into endpoints and manifest entries — and into no Functions bindings, so an Azure Functions deployment hand-writes what the compiler already knows. ADR-0077 names this generator as one of its three pieces; this package builds it against `Immediate`, not `Dispatched` | Generated `[Function]` entry points call WP-140's seam and contain no logic of their own — the mapping is Http→HttpTrigger, Bus→ServiceBusTrigger, Schedule→TimerTrigger firing `FlowScheduleScan.RunOnceAsync`, Stream→EventHubsTrigger, Change→TimerTrigger firing `FlowChangeScan.RunOnceAsync` (a scaled-to-zero host cannot LISTEN), Agent→HttpTrigger; Manual/Cli generate nothing. The host registers with `Sweeps = HostSweeps.None` — the platform is the scheduler now, and a sweep beside a TimerTrigger double-fires | A sample runs on the Functions **local runtime** end to end over at least Http + Schedule + Bus; the generated file compiles under `TreatWarningsAsErrors`; a flow with no trigger attribute generates nothing. **Shipped with two divergences.** (1) *Stream is not bound*: `AdmitAsync` takes a **closed** window, and closing one needs the open windows, watermark and checkpoint a scaled-to-zero host does not keep — the generated file carries the reason where the binding would be. (2) ~~*The local-runtime run is owed*~~ **Paid 2026-08-15**: Core Tools 4.6.0 installs from its GitHub release (the npm route's CDN is what the proxy refuses), and the sample ran end to end on the real Functions host against the Service Bus **emulator** and **Azurite** — HttpTrigger 200 with the journal row, ServiceBusTrigger consuming a real emulator message into `stock.reserve` Completed, TimerTrigger firing through Azurite's ScheduleMonitor. The direct-invocation tests written while the runtime was unreachable stay — they are the CI-runnable half |
| **WP-142** — notify, not poll — **shipped 2026-08-14** | The change feed is a 1 s cursor poll and the timer sweep a 10 s scan, so a flow-emits-flow-consumes chain pays ~1 s and a due `.Delay(1s)` waits up to 10 s — the largest latency item in the platform, bigger than everything WP-140/141 buys. The polls also burn one query per interval per node while idle | Notification **accelerates** the sweep and never replaces it: `NOTIFY` on outbox commit (and on a parked wake becoming due) wakes the waiting service early — the seam is the `Task.Delay(NextInterval(), …)` at `FlowChangeService.cs:59` and `FlowTimerService.cs:65` — and the interval remains as the correctness backstop, so a dropped notification costs latency, never a lost event. `LISTEN` needs a dedicated connection, which a transaction pooler cannot carry: the listener runs on the **direct** connection string and a deployment that only has the pooled one keeps today's behaviour, stated in the option's doc | Measured on the event-driven sample: emit→consume p99 drops from ~1 s to the low tens of ms with notify on; **chaos rig unchanged and green** with notify on and with the listener's connection killed mid-run; idle-load query count drops and the number is recorded |

| **WP-143** — takeover wakes at expiry — **shipped 2026-08-14; one divergence: the trigger announces the schema's earliest live expiry, not the row's own — a renewal extends, so a row-own instant either fires on healthy leases or follows the wrong lease, and the reasoning is at the code** | A dead node's instances wait ≈ `LeaseTtl` (30 s) **plus** up to `RecoveryScanInterval` (10 s) before another node takes over — the chaos rig measures takeover p99 at 43–48 s, and the scan half of that is pure waiting on a poll. The expiry instant is known the moment the lease is written, and nothing uses it | Same shape as WP-142's timer hint, extended to `SweepKind.Recovery`: lease acquisition/renewal announces `expires_at`, the listener arms the recovery sweep to wake **at** expiry rather than up to an interval after it. The interval stays as the backstop; absent signal is today's behaviour byte-for-byte; `LeaseTtl`/renewal defaults do not move — the renewal-traffic trade is a deployment's, not a default's | Chaos rig re-run: takeover p99 within `LeaseTtl` + a small ε rather than `LeaseTtl + interval`, with **0** duplicates against the guarantee, 0 lost, 0 orphans unchanged; signal absent → the old p99 shape returns |
| **WP-144** — admission bounds — **shipped 2026-08-14** | Nothing sheds. A burst is admitted at whatever rate the sweeps and push hosts can read, the journal saturates, and latency collapses instead of degrading. `flowx_trigger_rejected_total` is declared and produced by nothing — the same defect `_admitted_total` had before WP-140 | A configurable in-flight admission ceiling at the WP-140 seam, **default unlimited** — a hosting option must never change a running deployment's behaviour (the `HostSweeps.All` rule). Over the ceiling: Bus/Stream → `Requeue` (the broker holds the backlog; that is what it is for), HTTP → `429` with `Retry-After` as RFC 7807. `_rejected_total` gains its producer here with a `reason` label; a rejection is *not* an admission and must not double-count | A burst beyond the ceiling loses nothing — requeued deliveries redeliver and complete; admitted work's latency stays bounded while shedding; both counters' sum equals offered load in the test; ceiling unset → zero behaviour change, asserted |

Ordering: WP-140 → WP-141 (needs the seam) · WP-142 independent, can run in parallel.
WP-143 and WP-144 claimed 2026-08-14 after the first three shipped; 143 rides WP-142's
signal, 144 rides WP-140's seam, and they touch disjoint files so they run in parallel.
WP-140 and WP-142 shipped 2026-08-14, built in parallel and merged the same day; each
was falsified by a compiling mutation before merge, and the chaos rig ran green over a
schema carrying migration 0013's triggers — 0 duplicates against the guarantee, 0 lost,
0 orphans, both arms. WP-141 shipped later the same day — its two divergences are at its row.
Sizing every replica count downstream of these stays gated on
[item 23](#9-open-items-blocking-the-plan) — B7 has a number and no verdict, and optimising
against an unjudged number is guessing.
...
| 15 | **`StepLoopBenchmarks.BuildPlan`'s committed 520 B is not reproducible, and the gate treats allocation counts as exact and machine-independent.** Same commit, same runtime version, same warmup and iteration configuration: **456 B** on the container at `e6fcd37`, **464 B** on the container at `dev`, **528 B** on the GitHub runner at `dev`, **520 B** in the file. Every other allocation entry in those same runs agreed across both machines, `StepLoopBenchmarks.CompensateAll` to the byte at 440 B, so this is one entry rather than a broken harness — and `BuildPlan` builds a plan through Roslyn-adjacent machinery, which [WP-31](#wp-31--a-gate-that-catches-a-regression-while-the-budget-is-failing) already names the suspect shape for: *"Roslyn sizes some pools from `ProcessorCount`"*. ~~Options: (a)/(b)/(c)~~ **Closed 2026-08-15 as (b), with the suspect corrected.** The variance is not Roslyn — `BuildPlan` reaches no Roslyn and no `ProcessorCount`; it is the JIT's escape analysis stack-allocating one temporary, decided by tier-up/PGO timing per machine, reproduced by toggling `DOTNET_TieredPGO`/`JitObjectStackAllocation` alone: 464 B ↔ 528 B, same binary. Banded 464 B + 15 % by `CompilerBenchmarks`' mechanism; (a) was available only as pinning the JIT for the whole harness and is refused with the reason at the entry. *Recorded in passing and deliberately untouched: `run-benchmarks.sh` sets `DOTNET_TieredPGO=0` and `performance.yml` does not, so script and CI have never measured the same JIT.* The allocation half of the job is deterministic; what still holds the job red is the generator-cost p95 regression this table tracks separately | — | closed |
| 16 | **A declared wait the compiler cannot fold publishes no `timeout`, and nothing says so.** [ADR-0021](docs/adr/ADR-0021-manifest-publishes-the-wait.md) folds `.AwaitSignal<T>(timeout)`'s duration at build time and, when it cannot, publishes nothing — the precedent `merge` set, and right for `merge`. It is wrong here, and the two halves of WP-63 proved it against each other rather than in theory: the timer work made `samples/workflow`'s `Waits.Countersignature` an environment read so a demonstration would not wait seven days, which is unfoldable, so the merged sample **silently stopped publishing `timeout`** — `flowx diff` had no window to compare and ADR-0021's new field lost the only producer in the repository. That is ADR-0017's **F1** failure arriving by a route nobody had listed, and the build stayed green: only `Workflow.Tests.ManifestTests` caught it, because it is the one test that reads the field. The sample is restored to constants with the reason recorded at the declaration. ~~What is owed is the **decision**~~ **Decided and shipped 2026-08-14: the diagnostic.** `FLOWX1054` (warning — the flow still runs; what is lost is contract visibility, which is `FLOWX1043`'s ground for a warning, not an error) fires from inside `FoldDeclaredWait` itself, so the diagnostic and the fold can never disagree; both `timeout` producers (`AwaitSignal`, `PollUntil`) are covered, and the workflow-sample test asserts the published `"P7D"` before asserting silence, so it cannot pass vacuously | — | closed |
| 17 | **`flowx_trigger_admitted_total` is declared and nothing produces it.** *The documentation half is closed:* [18 §3](docs/18-Cloud-Native.md#3-autoscaling) printed an HPA rule reading `flowx_trigger_admitted_rate`, which could never have fired, and it now scales on ingress request rate with a caution naming the seven instruments that **do** have producers. [12 §3](docs/12-Observability.md#3-metrics) was already honest and needed nothing. **Decided 2026-08-14: emit it.** The admission seam it always lacked is [WP-140](#6c-cross-phase--serverless-and-latency-wp-140--wp-142)'s deliverable, and the counter's emission is written into that package's Done-when rather than tracked twice here | [WP-140](#6c-cross-phase--serverless-and-latency-wp-140--wp-142) | — |
| 18 | ~~**The CI link check verifies that a file exists and never that an anchor does.**~~ **Closed 2026-08-14.** `ci.yml`'s `docs` job now slugs every heading by GitHub's rules — link labels reduced to their text, backticks dropped — and fails on a fragment that resolves to nothing; proved falsifiable by breaking one anchor and watching it go red. All nineteen breaks are repaired: four in `24-Getting-Started`, `12-Observability`, the ADR index and `samples/banking/README`, and **six inside ADR-0026, ADR-0040 and ADR-0041 with the repository owner's agreement** — each named a heading in *another* document that had been renamed (*four* and *six of seven metrics emit* are both *seven of seven* now), so not one word of any record changed. **One exemption remains and is not a rename**: ADR-0020 references a section it does not contain, which is a content gap in an accepted record and has nowhere correct to point. The exemption is keyed to that exact (file, fragment) pair and self-expires | — | closed |
| 19 | **~~Two broker plugins ship and CI has executed almost none of them.~~ Half stale when re-read on 2026-08-14, and the executable half is now verified.** The row said the suites "skip for want of a live broker, which no workflow stands up" — `ci.yml`'s build job stands up Kafka (KRaft tarball), the Service Bus emulator, RabbitMQ and Redis, and sets all four connection variables; the skip counts quoted were a broker-less local machine's, not CI's. Verified against live brokers on 2026-08-14: **Kafka 22/22, RabbitMQ 33/33, Redis 95/95 — zero skips, zero failures.** What stays open is the Service Bus half of [28 §4](docs/28-Azure-Hosting.md#4-managed-services-every-topology-shares)'s gate: the emulator is not *a broker in a test subscription*. **Deferred by the repository owner on 2026-08-14: the emulator suffices for now; the real-Service-Bus run moves to the pre-production gate rather than blocking anything today** | Nothing until a production path; then the real run | repository owner |
| 20 | **`Dispatched` is decided and belongs to no phase.** [ADR-0077](docs/adr/ADR-0077-a-flow-is-dispatched-in-one-of-two-modes.md) accepts a second dispatch mode — one invocation per step, no lease, Durable only — and names the three pieces it needs: a `FlowX.Functions` binding generator, dispatched execution in the engine, and a Cosmos `IFlowJournal` that would retire both the connection ceiling and the outbox poller. None exists, no work package covers them, and [§6a](#6a-p4p9--what-this-plan-does-not-yet-contain) reserves ranges without naming them. Deliberately **not** given package numbers here: the phase that owns the work should allocate them, and detailing an unscheduled package is what §6a exists to avoid. **Decided 2026-08-14: no phase — deferred, and `Immediate` is the default in every environment.** The review found `Immediate` plus durable suspension already covers the serverless cases that matter: `Delay`/`AwaitSignal` park the instance in the journal holding no process, so a long wait costs nothing, and only a single step (or an unbroken active chain) outrunning the platform's invocation limit actually needs one-invocation-per-step — at the price of one journal round trip and one startup **per step**. ADR-0077 stays as written; the revisit trigger is a real workload hitting the invocation ceiling. [§6c](#6c-cross-phase--serverless-and-latency-wp-140--wp-142) builds the serverless path on `Immediate` | Nothing. Revisit on a workload that outruns an invocation limit | — |
| 21 | ~~**Transaction pooling breaks tenant isolation.**~~ **Fixed 2026-08-14, the day it was found.** `TenantScope` bound with `set_config(…, **false**)` — session-scoped — once per connection open, on an argument that holds for Npgsql's pool, where a client connection is a server session, and fails in front of a transaction-pooling proxy, where one server connection is shared between clients. Reproduced against PgBouncer 1.22 / PostgreSQL 16 at `default_pool_size = 1`: A binds `tenant-A`, B binds `tenant-B`, **A's next statement read `tenant-B`** — a cross-tenant read. The binding is now transaction-local and issued inside a transaction the scoped acquire opens; `ApplyAsync` takes the `NpgsqlTransaction`, so a bind with nothing to belong to cannot be written, and a `ScopedConnection` carries the transaction because the commit has to be reachable. **Unscoped deployments open no transaction and pay nothing.** Re-run on a clean pooled connection reads **`<EMPTY>`**, and the work still sees its own tenant inside its own transaction. 246/246 Postgres tests pass. *The **regression test** this owed landed on 2026-08-14 and is now green against a live PgBouncer 1.22 in `pool_mode = transaction`: `PooledTenantIsolationTests`, gated on `FLOWX_POSTGRES_POOLED_CONNECTION`, migrating a constant schema and putting it on the endpoint's role because a role default is the only route to a schema a shared server session carries. **Its first draft was sequential and passed against the defective binding** — a binding that outlives its transaction is only read by somebody else if somebody else runs in between — so the arms now run concurrently, and the pre-fix shape restored as a compiling mutation fails it on round 0. 249/249 Postgres tests, 0 skipped* | ~~The regression test~~, **and a second finding**: FlowX cannot run behind PgBouncer at all today. The adapter selects its schema with Npgsql's `SearchPath`, a PostgreSQL **startup parameter**; PgBouncer refuses the connection (`08P01: unsupported startup parameter: search_path`), and `ignore_startup_parameters = search_path` makes it accept the connection and discard the schema (`42P01: relation "flow_instance" does not exist`). Both reproduced 2026-08-14. The dangerous one is the second, because the application starts. This is what stops the regression test running: it skips naming which of the two it hit, so a run that proved nothing says so. [28 §4.1](docs/28-Azure-Hosting.md#41-the-connection-ceiling--the-one-that-bites) no longer makes PgBouncer mandatory | repository owner |
| 22 | **Phase 0's deployment assets exist; Phases 1 to 5 do not.** `deploy/azure/main.bicep` **compiles** — Container Apps, PostgreSQL Flexible Server with **password authentication disabled**, Static Web Apps, App Insights, and a user-assigned identity — and `.github/workflows/deploy-azure.yml` builds the image in ACR, previews with `--what-if` by default, then deploys, authenticating by OIDC. **No password exists anywhere in it.** *Neither has been run against a subscription, because this repository has none, and both say so where a reader will see it.* One thing the template deliberately cannot do is create the application's database role: registering the identity as a server administrator would let it drop every database, so `deploy/azure/README.md` §3 carries the single `pgaadauth_create_principal` statement instead. PgBouncer is absent for [B-6](CHECKLIST.md), HA for Phase 2, the broker for Phase 3 — each named in the template rather than omitted, so the diff that enables one is small. **Deferred by the repository owner on 2026-08-14: no Azure needed for now; the assets stay compiling and unrun** | Nothing until a subscription exists | repository owner |
| 23 | **B7 has a number and no verdict, and the Azure design rests on it.** The durability rig prices a durable step commit, but the budget is `15 ms @ 5 000 commits/s/node` and the recorded four-core run saturates at **4 103/s** — so the checker returns INCONCLUSIVE rather than a pass, correctly. Store-side p99 at that rate was 14.146 ms, inside the ceiling and not a measurement of the budget. Every replica count, scaling threshold and latency promise in [28](docs/28-Azure-Hosting.md) is downstream of two numbers this run cannot produce: what a tier you can afford sustains, and its p99 there. One afternoon against a real managed PostgreSQL closes it. **Deferred by the repository owner on 2026-08-14: local PostgreSQL stands in for now, so B7 stays INCONCLUSIVE by design; the run happens before any production sizing claim, not before** | 28's numbers stay estimates, accepted for now | repository owner |

| 24 | **A sub-flow's child invocation drops `Principal`, so an `Authenticated` step inside a sub-flow is refused.** Found 2026-08-15 while shipping Consent; pre-existing and latent — `samples/workflow` composes only `Internal` capabilities, so nothing living hits it. `Purpose` now propagates on that line (with the tenant/correlation group); `Principal` deliberately does not, because whether a parent's identity should flow into a child is a security decision, not a plumbing default | A sub-flow with a non-`Internal` step | repository owner |

**Three items in this table are live: 12, 24, and 21's B-6 remainder.** *Items 15 and 11's last clause closed 2026-08-15.* *Item 16 closed 2026-08-14 (FLOWX1054); items 19, 22 and 23 were deferred the same day at the repository owner's direction — emulators and local stores suffice until a production path needs Azure.* *The previous count read: seven — 12, 15, 16, 19, 21, 22, 23, plus 11.* *Items 17 and 20 were decided on 2026-08-14 — 17 lands as a producer inside [WP-140](#6c-cross-phase--serverless-and-latency-wp-140--wp-142), 20 is deferred with `Immediate` as the default everywhere — and both rows record their decision rather than being struck, because each still names a revisit condition.* *The previous count read: nine — 12, 15, 16, 17, 19, 20, 21, 22, 23, plus what is
left of 11.** *Item 21's defect is fixed; the row stays live for the regression test it still
owes.* *Item 18 closed on 2026-08-14, the day after it was opened.*
*Item 18's gate shipped the day after it was opened; the row is struck for the work and
still carries a decision, which is why it is counted.* *This line said "12, 13 and 14" while 13 and 14 were struck through two rows above
it, and it did not mention 15 or 16 at all. Items 17 to 23 were opened on 2026-08-14 out of
the Azure hosting review; every one of them is a decision or a measurement rather than a
build, which is why none carries a work-package number.* *This line read "**No item in this table is live**", which was already wrong
when it was written — item 11 was never struck through and carries a Blocks and an Owner —
...
**clause by clause** for the same reason: (b) and half of (c) are discharged and (a) is not.
*This sentence listed 11 among the fully struck rows while the row itself was live.*
