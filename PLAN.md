...
## 6c. Serverless and latency — WP-140 … WP-142
...
| 17 | **`flowx_trigger_admitted_total` is declared and nothing produces it.** *The documentation half is closed:* [18 §3](docs/18-Cloud-Native.md#3-autoscaling) printed an HPA rule reading `flowx_trigger_admitted_rate`, which could never have fired, and it now scales on ingress request rate with a caution naming the seven instruments that **do** have producers. [12 §3](docs/12-Observability.md#3-metrics) was already honest and needed nothing. **Decided 2026-08-14: emit it.** The admission seam it always lacked is [WP-140](#6c-serverless-and-latency--wp-140--wp-142)'s deliverable, and the counter's emission is written into that package's Done-when rather than tracked twice here | [WP-140](#6c-serverless-and-latency--wp-140--wp-142) | — |
...
**Seven items in this table are live: 12, 15, 16, 19, 21, 22 and 23, plus what is left of 11.** *Items 17 and 20 were decided on 2026-08-14 — 17 lands as a producer inside [WP-140](#6c-serverless-and-latency--wp-140--wp-142), 20 is deferred with `Immediate` as the default everywhere — and both rows record their decision rather than being struck, because each still names a revisit condition.* *The previous count read: nine — 12, 15, 16, 17, 19, 20, 21, 22, 23, plus what is
