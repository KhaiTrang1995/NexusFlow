...
| 21 | **Transaction pooling breaks tenant isolation. Verified on 2026-08-14, not suspected.** `TenantScope` binds with `set_config('flowx.tenant_id', @tenant, **false**)` — session-scoped — once per connection open. The safety argument written at that declaration is that a scoped journal rebinds on every open, so a pooled connection cannot carry one execution's tenant into the next; that is true of Npgsql's pool, where a client connection is a server session, and false of PgBouncer in transaction mode, where consecutive statements from one client can land on different server connections and one server connection is shared between clients. Reproduced against PgBouncer 1.22 / PostgreSQL 16 at `default_pool_size = 1`: A binds `tenant-A`, B binds `tenant-B`, then **A's next statement reads `tenant-B`**. The same three steps direct to PostgreSQL return empty, which is correct. **This is a cross-tenant read, the highest-severity class this platform has.** The fix is not a setting: the binding must be transaction-local (`set_config(…, true)`) and issued in the same transaction as the statement, which changes the adapter's execution path and must be proven by the journal and tenant conformance suites rather than patched. Deliberately not attempted in the commit that found it. **The fix, designed 2026-08-14 and not yet built.** `OpenAsync` begins a transaction **only when scoped**, so an unscoped deployment pays nothing and keeps today's single-round-trip reads, and binds `set_config(…, true)` inside it — transaction-local, therefore on the same server connection as the work and unable to outlive it. It returns a `ScopedConnection` readonly struct holding the connection and the optional transaction, exposing `Connection` and `CommitAsync`; disposal disposes the transaction then the connection, so an uncommitted write rolls back rather than half-lands. Seven call sites in `PostgresFlowJournal` and one in `PostgresSubjectErasure` change shape; the **two** that call `BeginTransactionAsync` today cannot nest, so they take the session's transaction when there is one and begin their own when there is not. Acceptance: the journal and tenant conformance suites pass unchanged, plus a new case that runs the tenant isolation suite against a **pooled** endpoint and fails on today's code. Npgsql enlists commands in the connection's current transaction automatically, so no command construction changes. Mitigation until then, recorded in [28 §4.1](docs/28-Azure-Hosting.md#41-the-connection-ceiling--the-one-that-bites): session pooling, or a direct connection with replicas bounded by `max_connections` | **Any multi-tenant deployment behind a transaction-pooling proxy** — which [28 §4.1](docs/28-Azure-Hosting.md#41-the-connection-ceiling--the-one-that-bites) otherwise makes mandatory at scale | repository owner |
| 22 | **There are no deployment assets for Azure, and 28 does not pretend otherwise.** No Bicep, no Terraform, no deployment workflow — the same gap [18](docs/18-Cloud-Native.md) records for Kubernetes, now with a second target. [28 §7](docs/28-Azure-Hosting.md#7-delivery-with-exit-criteria) gives six phases each with a checkable exit criterion, so the work is specified; nothing implements it. Recorded so that "we have an Azure design" is not read as "we can deploy to Azure" | Nothing builds. It is the distance between a document and a running system | repository owner |
...
left of 11.** *Item 18 closed on 2026-08-14, the day after it was opened.*
