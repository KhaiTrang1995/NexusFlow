...
| 18 | ~~**The CI link check verifies that a file exists and never that an anchor does.**~~ **Gate shipped 2026-08-14.** `ci.yml`'s `docs` job now slugs every heading by GitHub's rules — link labels reduced to their text, backticks dropped — and fails on a fragment that resolves to nothing; proved falsifiable by breaking one anchor and watching it go red. Four files were repaired: `24-Getting-Started` (*five* diagnostics became six), `12-Observability`, the ADR index and `samples/banking/README` (*four* and *six of seven metrics emit* are both *seven of seven* now). **What is owed is a decision, not work.** Eleven occurrences across **ADR-0026, ADR-0040 and ADR-0041** are exempted rather than repaired, because this repository does not edit accepted records and repointing a fragment is the owner's call to make — five of the six are the same one-word rename in another document, and the sixth is a section **ADR-0020** references and does not contain. Each exemption is keyed to an exact (file, fragment) pair and self-expires, so a different break in the same record still fails | Navigation inside four ADRs | repository owner |
...
**Ten items in this table are live: 12, 15, 16 and 17 to 23, plus what is left of 11.**
