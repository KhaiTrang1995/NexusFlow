...
| 17 | **`flowx_trigger_admitted_total` is declared and nothing produces it, while a document shows an autoscale rule reading it.** [18 §3](docs/18-Cloud-Native.md#3-autoscaling) prints a HPA rule on `flowx_trigger_admitted_rate`; that rule cannot fire, because the counter has no call site — `TelemetryNames` says so at the declaration, along with the `flowx.trigger.kind` and `flowx.trigger.source` attributes. The other seven instruments all have producers, verified. Two honest ends: emit it at the admission seam, or correct 18's example to a metric that exists. Not a third, which is leaving a copyable rule that silently never scales | An operator copying 18 §3 gets an autoscaler that never fires | repository owner |
| 18 | **The CI link check verifies that a file exists and never that an anchor does, and 19 anchors are already broken.** `ci.yml`'s `docs` job resolves every Markdown link to a path; a `#fragment` is discarded before the check. Nineteen links across `docs/12-Observability.md`, `docs/24-Getting-Started.md`, `ADR-0020` and `ADR-0026` point at headings that no longer exist — several at section titles that were renamed when a count in them changed, e.g. `#9-observing-policies--four-of-seven-metrics-emit` against a heading that now says six. The gap is one function: slug every heading in the target file and compare. Until it exists, a renamed heading breaks navigation silently, which is the same failure mode as an unchecked budget column | Navigation rots between reviews | repository owner |
...
**Ten items in this table are live: 12, 15, 16 and the new 17 to 23, plus what is left of
11.** *This line said "12, 13 and 14" while 13 and 14 were struck through two rows above
