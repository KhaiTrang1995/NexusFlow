...
| [FLOWX1022](FLOWX1022.md) | Step consumes a contract no earlier step produces | A flow that throws on its first request |
...
> **list** of transport namespaces, not a proof. `FLOWX1022` has stated limits for the
...
disjointness), `FLOWX1016` (expected failures are values), `FLOWX1019`–`FLOWX1021`
(deadline coherence, step binding, sub-flow cycles).
