...
| [FLOWX1032](FLOWX1032.md) | Declared policy is not executed by the runtime | **A step declaring a twenty-per-second rate limit and a twenty-four-hour idempotency window, published in the manifest as bounded by both and admitted whatever it declared.** *Its original example — a three-second timeout, three retries and a circuit breaker dispatched once — is now behaviour, as are the `Cache` and the `Audit` that replaced it in this column* |
