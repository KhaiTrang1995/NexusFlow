...
| A `Hedge`'s `afterDelay` | A hedged attempt's worst case is `(maxAttempts − 1) × afterDelay + timeout`, so from WP-79 the product understates a hedged step by `attempts × (maxAttempts − 1) × afterDelay`. Left uncounted deliberately: the extra term is a number the author wrote beside the timeout, and it is *below* the timeout in every declaration that does anything — an `afterDelay` at or above the `Timeout` is a hedge that can never fire, because the first call is already dead when the second would be issued. [ADR-0078 §2.5](../adr/ADR-0078-stage-four-nests-six-kinds.md) records the choice and makes it that record's first revisit condition. The deadline itself is still enforced: every timeout is clamped to what is left of it before it is armed |
