...
| Risk | Trigger | Action |
|---|---|---|
| R1 generator complexity | generator allocation above P1's per-flow or per-capability ceiling, or > 3 generator bugs per phase | freeze features; invest in the generator's test harness and model layer — **⚠ this trigger fired against the ratio the row used to name, and was answered on 2026-08-10.** [ADR-0014](adr/ADR-0014-derived-error-catalogue-vs-build-budget.md) kept the derived error catalogue and replaced the ratio with the per-unit ceilings above, which the generator meets (772,522 B/flow, 148,562 B/capability against 800,000 and 160,000). The wall-clock figures that fired it — +46.6 % at 50 flows, +67.1 % at 200 — stand as measurements of what they measured; what changed is that a ratio whose denominator is the user's code was the wrong instrument. The blocking *relative* gate ([generator-cost-gate.md](benchmarks/generator-cost-gate.md)) stays, and `check-generator-cost.py` now fails on the ceilings too rather than reprinting them |
