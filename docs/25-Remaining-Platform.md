...
| — | **`Schema` / `Database` isolation** | **built 2026-08-02** | `Schema` is built: a bounded connection pool per tenant, each pointed at that tenant's own schema by its connection string, provisioned and migrated on first use, with both sweeps, the outbox publisher and the change feed fanned out across schemas — a change carries the tenant whose schema and emitting instance it was read from, which is what starts its flow in that tenant ([ADR-0054](adr/ADR-0054-a-platform-trigger-attests-its-tenant.md)). `Database` stays refused, and [ADR-0051](adr/ADR-0051-database-isolation-is-a-topology-not-a-runtime-level.md) is why: it names a deployment per tenant, whose pod declares `None` |
| — | **Journal write budget per tenant** | **built 2026-08-02** | [16 §4](16-Multi-Tenant.md)'s sixth fairness mechanism, and [ADR-0057](adr/ADR-0057-a-write-budget-is-drawn-in-blocks-and-paces-rather-than-refuses.md) is the third option. A node draws a block of write credit from the shared bucket in one call and spends it locally, so the fleet total is the bucket's and the per-row cost is an interlocked decrement; an exhausted tenant is paced rather than refused, because a refused commit mid-flow strands a saga. It cannot be charged at admission: a `ForEach`, a retry and a backward jump all write rows the plan does not count |
| — | **`Stream` trigger** | **built 2026-08-02** | Bound end to end: `[StreamTrigger]` → `TriggerReader` → `StreamEmitter` → `FlowXStreamSubscriptions.g.cs` → `FlowStreamScan`. All eight trigger kinds now reach a running transport |
| — | **Stream engine** | **built 2026-08-02** | Tumbling event-time windows, a bounded channel, an observed watermark and a checkpointed source position. Window state is deliberately not journaled: a closed window derives the instance id it starts, so a rebuild is refused by `flow_instance`'s primary key rather than aggregated twice ([ADR-0055](adr/ADR-0055-a-window-names-the-instance-it-starts.md), [ADR-0056](adr/ADR-0056-the-watermark-is-observed-never-wall-clock.md)) |
