...
| — | **Journal write budget per tenant** | **built 2026-08-02** | [16 §4](16-Multi-Tenant.md)'s sixth fairness mechanism, and [ADR-0055](adr/ADR-0055-a-write-budget-is-drawn-in-blocks-and-paces-rather-than-refuses.md) is the third option. A node draws a block of write credit from the shared bucket in one call and spends it locally, so the fleet total is the bucket's and the per-row cost is an interlocked decrement; an exhausted tenant is paced rather than refused, because a refused commit mid-flow strands a saga. It cannot be charged at admission: a `ForEach`, a retry and a backward jump all write rows the plan does not count |
