...
| — | **`Schema` / `Database` isolation** | **built 2026-08-02** | `Schema` is built: a bounded connection pool per tenant, each pointed at that tenant's own schema by its connection string, provisioned and migrated on first use, with both sweeps fanned out across schemas. `Database` stays refused, and [ADR-0051](adr/ADR-0051-database-isolation-is-a-topology-not-a-runtime-level.md) is why: it names a deployment per tenant, whose pod declares `None` |
