...
| `InternalCapabilitiesAreNotAgentReachable` (LLM08) | not written, and **no longer vacuous — the property does not hold.** An agent surface now exists, and `Authorization.Internal` is enforced nowhere: `StepAuthorization.Decide` returns a permit for it and `CanRefuse` is false, so a flow stepping through an `Internal` capability runs for an anonymous caller down *any* transport. `Authorization`'s own summary claims two controls — "the Trigger Engine rejects it at admission" and "the compiler excludes it from the generated agent tool surface" — and neither is built. This is inherited rather than introduced by the agent surface, and it is now reachable. Deliberately **not** fixed inside `FlowX.Mcp`: excluding a tool from `tools/list` would be a separate authorisation path for agents, which is the one thing [25 §3](25-Remaining-Platform.md) forbids. The fix belongs in the step loop, where every transport gets it at once | a decision on `Internal`: an ADR, then `StepAuthorization` |
