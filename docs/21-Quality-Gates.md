...
|---|---|---|
| `CrossTenantAccessIsDenied` | **P3 only, and no longer P4.** *This cell read "**P4** — no policy executes at runtime, so no stage exists that could return `Forbidden`. `TenantId` is resolved from claims and carried on the invocation, and nothing consumes it." Both halves expired on 2026-08-02:* `ITenantResolver` refuses at admission with `ErrorCategory.Forbidden`, and PostgreSQL row-level security refuses at the row ([ADR-0046](adr/ADR-0046-a-tenant-is-resolved-at-admission.md)). What remains blocked is only **"across every trigger kind"**, which needs **P3** — HTTP is the one transport that supplies a tenant, and a bus or schedule trigger is *refused* rather than resolved, because the sources [16 §3](16-Multi-Tenant.md#3-tenant-resolution) gives them are not built. *An earlier correction in this cell still stands: WP-52's journalling of `tenant_id` is an execution record rather than an audit event.* | **The denial itself, in both directions, against a real database.** `TenantIsolationTests` and `TenantHostIsolationTests` assert what tenant *A* cannot reach of tenant *B*'s — instance row, resume frontier, fence, step commit, recovery candidate, timer candidate and host resume — and `TenantAdmissionTests` asserts that a deployment declaring isolation refuses an untenanted call, distinguishably from one that declares none. What is *not* yet assertable is the same across a second transport. |
...
| `AgentSurfaceEqualsFlowSurface` (LLM01, LLM07) | **enforced, not written under that name.** *This row said there was no agent surface.* There is one — `plugins/FlowX.Mcp` — and the equality it names is a start-up invariant rather than a test: `McpServer` refuses to construct when the manifest's `[AgentTrigger]` flows and the host's bindings are not the same set, in either direction, so a host that publishes an unservable tool or serves an unpublished flow never becomes ready. `McpServerBindingTests` drives both halves and `ToolDescriptorsCannotDriftTests` drives the descriptors' content | — |
| `InternalCapabilitiesAreNotAgentReachable` (LLM08) | **will not be written: the property it names is false, and making it true would break the trigger model** ([ADR-0047](adr/ADR-0047-internal-is-a-composition-stance.md)). An `Internal` capability *is* agent-reachable, transitively, exactly as it is HTTP-reachable — `samples/workflow/OnboardEmployeeFlow` is `[HttpTrigger]`ed and steps through three of them, so any reading that refuses an externally triggered flow reaching an `Internal` capability refuses the repository's own reference application. A tool is a **flow**, so there is no capability on the surface to exclude, and `FlowX.Mcp` filters nothing — [25 §3](25-Remaining-Platform.md) rules out a second authorisation path for agents. What is asserted instead is the premise the stance rests on: `NoTriggerAttributeAddressesACapability` and `EveryTriggerAttributeIsNamedForTheConventionTheGateMatches` (`SecurityFitnessTests`) fail the build the day a capability becomes directly addressable | — |
...
|---|---|---|---|
| **A01** | Broken access control | Authorisation is a **required member** on `[Capability]`, so a capability cannot compile without a stance — nor, since [`FLOWX1030`](diagnostics/FLOWX1030.md), with a `Permission` or `Policy` stance that names no permission or policy. Enforcement is at the business operation, not the route, so it holds over HTTP, Kafka and the agent surface alike. `Authorization.Internal` is a **composition** stance rather than a reachability control: it says the capability is never addressed on its own, which holds because a trigger addresses a flow, and it therefore admits every caller at the step ([ADR-0047](adr/ADR-0047-internal-is-a-composition-stance.md)). `Authorization.Policy` cannot be evaluated by the engine and so **refuses** rather than permitting. | `FLOWX1010`, `FLOWX1030`, `FLOWX1037` (build) · `EveryCapabilityDeclaresAuthorization`, `PublicCapabilitiesAreReviewed`, `NoTriggerAttributeAddressesACapability` (merge) · `CrossTenantAccessIsDenied` — **not yet enforced, see §2.4** |
...
| **LLM08** | Excessive agency | `ConfirmationMode.RequiredForSideEffects` is the default, and confirmation prompts state the **declared** side effects rather than a generic warning. What an agent may reach is the **flow** set the manifest publishes as tools — `Authorization.Internal` is *not* a way to hide a capability from an agent, and never was ([ADR-0047](adr/ADR-0047-internal-is-a-composition-stance.md)); a published flow's `Internal` steps run for whoever called it. Narrow the published flows, or put a refusing stance on a step. | `AgentSurfaceEqualsFlowSurface` — enforced as a start-up invariant by `McpServer`, see §3 |
...
| Tenant fairness | one tenant at 10× quota degrades another's p99 by ≤ 10 % | Release | **not written** — and the reason has changed. §4's mechanisms exist now: `FlowXOptions.Fairness` bounds a tenant's rate, quota and concurrency at admission, and `TenantFairShare` plus `PerTenantLimit` on the two sweep queries stop a tenant with a long backlog owning every page and every slot. `TenantFairnessTests` proves the starvation and then its absence — a quiet tenant that gets no slot in five sweeps gets one in the first. What is still absent is this row: **nothing measures a p99 under load**, so the ≤ 10 % figure is unverified and no job asserts it. **P6** |
