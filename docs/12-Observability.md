...
Plus the policy metrics from [10 §9](10-Policy-Framework.md#9-observing-policies--four-of-seven-metrics-emit).
**Four of those seven are emitted** — `flowx_policy_invocations_total`,
`flowx_retry_attempts_total`, `flowx_circuit_state` and `flowx_bulkhead_queue_depth` — because
[ADR-0025](adr/ADR-0025-a-partial-policy-engine-executes-stage-four-alone.md) executes
`PolicyStage.Resilience` and [ADR-0026](adr/ADR-0026-policy-metrics-name-only-what-executes.md)
gave the four decisions it makes an instrument. The remaining three —
`flowx_ratelimit_rejected_total`, the cache hit/miss pair and
`flowx_idempotency_replays_total` — have **no instrument at all**, for the reason
`flowx_stream_lag_records` has none: their policies do not run, so a rate-limit rejection or a
cache hit is not merely uncounted, it does not occur, and a permanently-zero series would
report that it never happens rather than that it cannot.
