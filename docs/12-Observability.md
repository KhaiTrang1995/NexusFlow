...
| `flowx_trigger_admitted_total` / `_rejected_total` | counter | `kind`, `reason`, `tenant` | **`_admitted_total` is emitted** by the admission seam — `FlowBusScan.AdmitAsync` and `FlowStreamScan.AdmitAsync`, which decide what becomes of one item without settling it and which a sweep and a push host both reach, so `kind` separates `Bus` from `Stream` and `reason` is `started` or `deduplicated`. *This row read "not emitted, and nowhere to emit from … a `kind` label needs one admission point serving every transport, and there is one transport"; **WP-140** built that admission point.* An item the seam requeued or dead-lettered was not admitted and is not counted here. ***`_rejected_total` is emitted too, since WP-144.*** *This row read "**`_rejected_total` still has no instrument**, because whether a requeue belongs in the same series as a poison message is a decision nobody has made". That was true of the seam's first two refusals and stopped being the whole list when `FlowXOptions.MaxInFlightAdmissions` added a third:* an item the node was **offered and decided not to run**, at a ceiling it set itself. That is neither a requeue the broker will re-offer for its own reasons nor a poison message, so it needed no ruling on either — `reason` is `shed`, and the two questions the old text raised are still open and still unanswered. A shed does **not** increment `_admitted_total`, so the two series sum to the offered load rather than overlapping |
...
Plus the policy metrics from [10 §9](10-Policy-Framework.md#9-observing-policies--seven-of-seven-metrics-emit).
**Six of those seven are emitted** — `flowx_policy_invocations_total`,
`flowx_retry_attempts_total`, `flowx_circuit_state`, `flowx_bulkhead_queue_depth` and the cache
hit/miss pair — because
[ADR-0025](adr/ADR-0025-a-partial-policy-engine-executes-stage-four-alone.md) executes
`PolicyStage.Resilience`, [ADR-0026](adr/ADR-0026-policy-metrics-name-only-what-executes.md)
gave the four decisions it makes an instrument, and
[ADR-0044](adr/ADR-0044-a-cache-is-a-plugin-store-keyed-by-the-redacted-input.md) executes stage
5. The remaining two — `flowx_ratelimit_rejected_total` and
`flowx_idempotency_replays_total` — have **no instrument at all**, for the reason
`flowx_stream_lag_records` has none: their policies do not run, so a rate-limit rejection is not
merely uncounted, it does not occur, and a permanently-zero series would report that it never
happens rather than that it cannot.
...
> `flowx_stream_lag_records` and `_rejected_total` are left without one.* **Thirteen, since
> **WP-144** gave `_rejected_total` a subject — the in-flight ceiling — so
> `flowx_stream_lag_records` is the last row without an emitter, and it is still waiting on
> **P7** rather than on anybody's decision.**
>
...
| Duplicated effects | `flowx_idempotency_replays_total` = 0 with retries > 0 | a capability declaring `Idempotent = true` but not honouring the key — **this row still does not work, and the reason narrowed rather than went away: `flowx_idempotency_replays_total` is one of the two [10 §9](10-Policy-Framework.md#9-observing-policies--seven-of-seven-metrics-emit) rows with no instrument, because stage 3 is not executed ([ADR-0025 §2.2](adr/ADR-0025-a-partial-policy-engine-executes-stage-four-alone.md)). The "retries > 0" half of the signal *does* work now — `flowx_retry_attempts_total` is emitted — so half this row is live and the half that would prove the duplicate is not** |
...
| Memory growth | `flowx_bulkhead_queue_depth`, stream channel depth | backpressure not reaching the source — **the first half works now**: the gauge is emitted whenever a caller queues for a bulkhead permit ([ADR-0026](adr/ADR-0026-policy-metrics-name-only-what-executes.md)), though an uncontended pool publishes nothing rather than a zero. The second still has no subject (P7) |
