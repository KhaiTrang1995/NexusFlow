# ADR-0027: A missed schedule fires late, inside a bounded catch-up horizon
...
> [ADR-0028](ADR-0028-a-scheduled-flows-input-is-its-occurrence.md) exists and had to be decided
> alongside this.
...
[ADR-0026](ADR-0026-an-occurrence-names-the-instance-it-starts.md) supplies that as a
...
- **C. A durable per-schedule cursor — a `flowx_schedule` table with `last_fired_at`.**
  *Rejected*, and it is [ADR-0026 §3](ADR-0026-an-occurrence-names-the-instance-it-starts.md)'s
...
- **A late firing runs with the occurrence's data and the present's world.** `ScheduledFire`
  carries the instant that was due ([ADR-0028](ADR-0028-a-scheduled-flows-input-is-its-occurrence.md)),
