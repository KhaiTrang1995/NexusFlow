...
| [0026](ADR-0026-policy-metrics-name-only-what-executes.md) | The four [10 §9](../10-Policy-Framework.md#9-observing-policies--specification) rows whose policy executes get an instrument and the three whose policy does not are left unnamed; `flowx_circuit_state` is a synchronous gauge recorded on transition rather than an observable one polled per scrape | Accepted | a §9 row's policy starts executing, at which point its name is frozen and this record's omission becomes an addition; or a second engine needs the breaker state, which a per-transition gauge cannot serve without a registry of live engines |
