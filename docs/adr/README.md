...
| [0025](ADR-0025-a-partial-policy-engine-executes-stage-four-alone.md) | Stage 4 executes in full; stages 1, 3 and 5 do not, and each skip is argued separately — the duplicate-charge row of 0011's table is held shut by FLOWX1014 and a stable idempotency key, not by the stage-3 policy (amends 0011) | Accepted | stage 1, 3 or 5 is implemented, at which point FLOWX1032 narrows again; `Hedge` or `Fallback` becomes declarable, which would falsify "stage 4 in full"; or FLOWX1014 is downgraded from an error, which removes the first of the two mechanisms |
