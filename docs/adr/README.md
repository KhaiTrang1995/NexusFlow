...
| [0020](ADR-0020-cli-reads-the-journal-as-rows.md) | `flowx replay --mode inspect` reads the journal as rows over the published migration contract and joins it against the manifest; no journal document is published and `CliDependsOnNothingButTheManifest` is not amended | Accepted | a second journal store adapter ships; or a `replay` mode has to *execute* rather than render, which this record explicitly does not decide; or the manifest freeze (0017) closes with `replay` still reading columns nothing versions |
