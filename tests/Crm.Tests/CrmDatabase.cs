...
/// <remarks>
/// <para>
/// <strong>A silently skipped test is indistinguishable from a passing one.</strong> Almost
/// everything this project asserts is a property of the database rather than of the sample's
/// code: row-level security under a role that cannot bypass it, a trigger that refuses a
/// dangling polymorphic reference, a partial unique index, a view with invoker rights. Every
/// one of them would pass vacuously against a dictionary and none of them can be checked
/// without a server.
/// </para>
/// <para>
/// So the decision is noisy in the same three directions <c>FlowX.Postgres.Tests</c>'s own
/// probe is, and the wording is deliberately its wording. No server configured: every test
/// skips, carrying this class's reason. A server configured but unreachable: every test
/// <em>fails</em>, because that is the case that looks like success in CI — somebody wired a
/// service container, the variable is set, the container did not come up, and a skip would
/// report a green isolation suite that never opened a connection.
/// </para>
/// </remarks>
internal static class CrmDatabase
{
    ...
    public static InvalidOperationException Unreachable() => new(
        ...
        "the CRM's row-level security, its polymorphic integrity trigger and its process " +
        $"versioning as green against a database that was never reached. {Reason}");
    ...
    private static Probe Run()
    {
        ...
        if (connectionString is null)
        {
            return new Probe(
                ...
                "and nothing about this sample's schema or its isolation has been verified. " +
